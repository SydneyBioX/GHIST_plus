...
class Framework(nn.Module):
    ...
    def __init__(
        ...
        hurdle_cfg = _to_namespace(getattr(model_cfg, "hurdle", None))
        self.hurdle_enabled = bool(getattr(hurdle_cfg, "enabled", False))
        ...
        self.attention_ablation_off = bool(
            getattr(model_cfg, "attention_ablation_off", False)
        )
        self.reference_ablation_off = bool(
            getattr(model_cfg, "reference_ablation_off", False)
        )
        self.ref_temperature = float(getattr(model_cfg, "avgexp_temp", 1.0))
        self.reference_mix_temperature = float(
            getattr(model_cfg, "reference_mix_temperature", 1.0)
        )
        if (
            not math.isfinite(self.reference_mix_temperature)
            or self.reference_mix_temperature <= 0.0
        ):
            raise ValueError("model.reference_mix_temperature must be finite and > 0")
        ...
        # Legacy models retain their original clamp. HurdleFramework exposes
        # the signed Eq. 5 location and applies exactly one final ReLU itself.
        self.expr_relu = bool(getattr(model_cfg, "expr_relu", True))
        ...
        self.foundation_adapter_trainable = (
            self.use_foundation_model
            and bool(getattr(fm_cfg, "train_adapter", False))
        )
        self.foundation_adapter_scope = str(
            getattr(fm_cfg, "adapter_scope", "all")
        ).lower()
        if (
            self.foundation_adapter_trainable
            and self.foundation_adapter_scope not in {"all", "h1"}
        ):
            raise ValueError(
                "foundation_model.adapter_scope must be 'all' or 'h1'"
            )
        ...
        self.freeze_backbone = (
            self.legacy_backbone_frozen
            or (self.use_foundation_model and not self.foundation_adapter_trainable)
        )
        if self.use_foundation_model:
            ...
            if self.foundation_adapter_trainable:
                trainable_modules = (
                    adapter_modules
                    if self.foundation_adapter_scope == "all"
                    else (self.cnn.proj_h1,)
                )
        ...
        if self.freeze_backbone:
            self.cnn.eval()
        elif self.foundation_adapter_trainable:
            self.cnn.enc.eval()
        ...
        self.vq_patch_ablation_off = bool(
            getattr(vq_cfg, "ablation_off", False)
        )
        ...
        if self.use_avgexp:
            ...
            if self.use_avgexp_residual:
                ...
                if self.hurdle_enabled:
                    # Scratch hurdle runs start at the full reference baseline.
                    # The zero output layer receives gradients on step one.
                    for residual_head in (
                        self.mlp_avgexp_residual,
                        self.mlp_avgexp_residual_immune,
                        self.mlp_avgexp_residual_invasive,
                    ):
                        nn.init.zeros_(residual_head.mlp[-1].weight)
                        nn.init.zeros_(residual_head.mlp[-1].bias)
        ...
        self.ecrm_ablation_off = bool(getattr(ecrm_cfg, "ablation_off", False))
        ...
        if self.use_ecrm:
            ...
            if self.ecrm_residual_target and not self.hurdle_enabled:
    ...
    def _build_hurdle_occurrence_graph_state(
        ...
        """Optional hurdle-only graph context built after magnitude is complete."""
    ...
    def _resolve_ref_weights(self, learned_weights, ct_prob_pred, ct_labels):
        if not (self.use_celltype and self.use_celltype_priors):
            return self._apply_reference_mix_temperature(learned_weights)
        if self.use_gt_ct_ref_weights and ct_labels is not None:
            resolved = F.one_hot(ct_labels, num_classes=self.n_ref).float()
            return self._apply_reference_mix_temperature(resolved)
        ...
        resolved = mixed / mixed.sum(dim=1, keepdim=True).clamp_min(1e-6)
        return self._apply_reference_mix_temperature(resolved)

    def _apply_reference_mix_temperature(self, weights):
        """Temper a resolved reference simplex; T=1 is an exact identity."""

        temperature = float(self.reference_mix_temperature)
        if temperature == 1.0:
            return weights
        tempered = weights.clamp_min(0.0).pow(1.0 / temperature)
        return tempered / tempered.sum(dim=1, keepdim=True).clamp_min(1e-6)
    ...
    def _graph_residual_from_morphology(
        self, self_state, graph_state, self_direct=None
    ):
        """Shared-head graph correction; identity graph is exactly zero."""

        graph_raw, _ = self.mlp_avgexp_residual(graph_state)
        graph_direct = self._scale_avgexp_residual(graph_raw)
        if self_direct is None:
            self_raw, _ = self.mlp_avgexp_residual(self_state)
            self_direct = self._scale_avgexp_residual(self_raw)
        delta = graph_direct - self_direct
    ...
    def _fuse_direct_with_ref(self, expr_direct, ref_base, ref_offsets, gate_raw, gate_max):
        if self.hurdle_enabled:
            # Eq. 4 is the baseline, not a gated correction. Keep the result
            # signed here; HurdleFramework owns the one final ReLU.
            gate = ref_base.new_tensor(1.0)
            out_expr = ref_base + expr_direct
    ...
    def _freeze_encoder(self, trainable_blocks: int = 0):
        encoder = (
            self.cnn.enc
            if self.use_foundation_model and hasattr(self.cnn, "enc")
            else self.cnn
        )
        ...
        if blocks is None:
            logging.warning("Uni2H blocks not found; encoder remains frozen")
            return
    ...
    def forward(
        ...
        allow_composition = (not self.vq_patch_ablation_off) and (
            (not self.vq_patch_composition_requires_vq)
            or self.vq_patch is not None
        )
        ...
        if (
            ...
            and not self.vq_patch_ablation_off
        ...
        if not allow_composition and not self.vq_patch_ablation_off:
        ...
        explicit_empty_graph = (
            isinstance(cell_edge_index, torch.Tensor)
        ...
        graph_context_state = None
        if self.use_avgexp:
            # assume reference provided when avgexp is enabled
            ref = ref_orig.unsqueeze(0).to(device)
            ref = ref.expand(embeddings.size(0), -1, -1)
            if self.reference_ablation_off:
                # Retain the reference path's topology while making all three
                # expression branches invariant to scRNA reference values.
                # Refiners still run, but receive an exact zero reference.
            ...
            if (
                self.use_crossattn
                and not self.attention_ablation_off
                and comp_tiled_all is not None
            ):
            ...
            if (
                ...
                and (
                    (self.hurdle_enabled and hasattr(self, "mlp_avgexp_residual"))
                    or self.ecrm_graph_residual_head is not None
                )
                and not (self.hurdle_enabled and self.ecrm_ablation_off)
                and not (self.hurdle_enabled and explicit_empty_graph)
                and coords_all.size(0) > 1
                ...
                if self.hurdle_enabled:
                    graph_residual_delta = self._graph_residual_from_morphology(
                        embeddings,
                        graph_context_state,
                        self_direct=expr_direct,
                    )
                else:
                    self.ecrm._patch_ids = patch_assign
                    ...
                    graph_expr_contrast = torch.nan_to_num(
                        graph_expr_contrast, nan=0.0, posinf=0.0, neginf=0.0
                    )
                    graph_expr_contrast_feat = self.ecrm_expr_contrast_proj(
                        graph_expr_contrast
                    )
                    graph_features = torch.cat(
                        [
                            embeddings_gate,
                            graph_context_state,
                            graph_context_state - embeddings_gate,
                    ...
                    graph_gene_gate = (
                        ...
                        * torch.sigmoid(self.ecrm_graph_gene_gate_raw)
                        .to(
                            device=graph_residual_delta.device,
                            dtype=graph_residual_delta.dtype,
                        )
                        .view(1, -1)
                    )
                    graph_residual_delta = graph_residual_delta * graph_gene_gate
                    torch.nan_to_num_(
                        graph_residual_delta, nan=0.0, posinf=0.0, neginf=0.0
                    )
        ...
        occurrence_graph_state = None
        if self.hurdle_enabled:
            occurrence_graph_state = self._build_hurdle_occurrence_graph_state(
                embeddings_gate,
                coords_all,
                edge_index_cells,
                patch_assign,
        ...
        self.last_aux_losses = {
            ...
            "ecrm_self_state": embeddings_gate if self.hurdle_enabled else None,
            "ecrm_graph_state": graph_context_state if self.hurdle_enabled else None,
            "ecrm_explicit_empty_graph": bool(explicit_empty_graph),
            "occurrence_ecrm_graph_state": occurrence_graph_state,
        }
