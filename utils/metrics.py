...
def figure3_giotto_scores_and_order(expression, coordinates, k=8):
    ...
    ``expression`` is already ``log1p(raw counts)``.
    """
    ...
    order = np.argsort(
        np.nan_to_num(scores, nan=-np.inf), kind="mergesort"
    )[::-1]
    return scores, order.astype(np.int64, copy=False)
...
def log_svg_validation_epoch(metrics: dict, *, epoch: int, svg_topk=(20, 50)):
    """Emit one concise line containing the VAL checkpoint metrics."""

    summary = (
        metrics.get("fixed_gt_svg_validation")
        if isinstance(metrics, dict)
        else None
    )
    if not isinstance(summary, dict):
        logging.info("VAL epoch=%d SVG metrics unavailable", int(epoch))
        return
    ...
    for k_value in svg_topk:
        values = summary.get(f"top{int(k_value)}", {})
        requested = int(values.get("requested_gene_count", int(k_value)) or 0)
        parts.append(
            "SVG{0} PCC(med/max/min)={1}/{2}/{3}[{4}/{5}] "
            "SSIM(med/max/min)={6}/{7}/{8}[{9}/{5}] "
            "CMD(med/max/min)={10}/{11}/{12} full={13}".format(
                int(k_value),
                _number(values.get("pcc_median")),
                _number(values.get("pcc_max")),
                _number(values.get("pcc_min")),
                int(values.get("pcc_valid_gene_count", 0) or 0),
                requested,
                _number(values.get("ssim_median")),
                _number(values.get("ssim_max")),
                _number(values.get("ssim_min")),
                int(values.get("ssim_valid_gene_count", 0) or 0),
                _number(values.get("cmd_median", values.get("cmd"))),
                _number(values.get("cmd_max")),
                _number(values.get("cmd_min")),
                "yes" if values.get("full_k_of_k") else "no",
            )
        )
    logging.info("VAL epoch=%d | %s", int(epoch), " | ".join(parts))
