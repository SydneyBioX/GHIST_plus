...
LEGACY_HISTOLOGY_NORMALIZATION = "legacy_slide_zscore"
UNI2_HISTOLOGY_NORMALIZATION = "uni2_imagenet"
_UNI2_RGB_MEAN = (0.485, 0.456, 0.406)
_UNI2_RGB_STD = (0.229, 0.224, 0.225)


def resolve_foundation_model_input_normalization(opts_data):
    mode = getattr(
        opts_data,
        "foundation_model_input_normalization",
        LEGACY_HISTOLOGY_NORMALIZATION,
    )
    if mode is None:
        mode = LEGACY_HISTOLOGY_NORMALIZATION
    ...
    if mode not in allowed:
        raise ValueError(
            "data.foundation_model_input_normalization must be one of "
            f"{sorted(allowed)}, got {mode!r}"
        )
    return mode


def validate_foundation_model_input_normalization(opts_data, model_cfg):
    mode = resolve_foundation_model_input_normalization(opts_data)
    foundation_cfg = getattr(model_cfg, "foundation_model", None)
    foundation_enabled = bool(getattr(foundation_cfg, "enabled", True))
    if mode == UNI2_HISTOLOGY_NORMALIZATION and not foundation_enabled:
        raise ValueError(
            "data.foundation_model_input_normalization='uni2_imagenet' "
            "requires model.foundation_model.enabled=true"
        )
    return mode


def normalize_uni2_histology_tensor(hist_torch):
    """Apply the official UNI2-H RGB tensor normalization contract."""
    if not isinstance(hist_torch, torch.Tensor):
        raise TypeError("hist_torch must be a torch.Tensor")
    if hist_torch.ndim != 3 or hist_torch.shape[0] != 3:
        raise ValueError("UNI2-H normalization expects a [3,H,W] RGB tensor")
    hist_torch = hist_torch.to(dtype=torch.float32) / 255.0
    mean = hist_torch.new_tensor(_UNI2_RGB_MEAN).view(3, 1, 1)
    std = hist_torch.new_tensor(_UNI2_RGB_STD).view(3, 1, 1)
    return (hist_torch - mean) / std


def normalize_histology_tensor_after_joint_transform(hist_torch, mode):
    """Keep legacy tensors exact; normalize raw RGB only for the opt-in path."""
    if mode == LEGACY_HISTOLOGY_NORMALIZATION:
        return hist_torch
    if mode == UNI2_HISTOLOGY_NORMALIZATION:
        return normalize_uni2_histology_tensor(hist_torch)
    raise ValueError(f"Unknown histology normalization mode: {mode!r}")
...
def _resolve_max_cells_per_patch(configured_value):
    ...
    if configured_value <= 0:
        raise ValueError("data.max_cells_per_patch must be positive")
...
def get_input_data(
    ...
    if fp_cell_type is not None:
        ...
        if not is_all_numbers:
            ...
            logging.debug("Cell type data shape: %s", df_ct.shape)
        df_ct["ct"] = df_ct["ct"] + 1
    else:
        df_ct = None
    ...
    if cache_path.exists():
        ...
        logging.info("Dataset metadata cache hit: %s", cache_path)
        return (
            coords_starts_valid,
            hist,
            nuclei,
    ...
    logging.debug("Histology image=%s nuclei=%s", hist.shape, nuclei.shape)
    ...
    logging.debug("Eligible cells: %d", n_cells)
    ...
    logging.debug(
        "Patch vertical extent: %d..%d", h_starts.min(), h_starts.max() + hsize
    )

    # check there are cells in the patches
    logging.debug("Scanning candidate patches for valid cells")
    ...
    # else:
    for hs, ws in tqdm(coords_starts, desc=f"prepare-{mode}-patches"):
    ...
    logging.debug("Loading or computing histology standardisation")
    ...
    if mode == "train":
        if not os.path.exists(fp_norms):
            ...
            for hs, ws in tqdm(
                coords_starts, desc=f"standardise-{mode}-patches"
            ):
    ...
    logging.info(
        "Dataset prepared: mode=%s cells=%d valid_patches=%d cache=%s",
        mode,
        n_cells,
        len(coords_starts_valid),
        cache_path,
    )
...
class DataProcessing(data.Dataset):
    def __init__(
        ...
        logging.debug("Stain normalisation enabled: %s", self.normstain)
        ...
        self.foundation_model_input_normalization = (
            resolve_foundation_model_input_normalization(opts_data)
        )
        self.device = device
        self.experiment_path = experiment_path
        self.stain_aug = stain_aug
        source_paths = {
            name: getattr(opts_data_sources, name, None)
            for name in (
                "fp_nuc_seg",
                "fp_hist",
                "fp_nuc_sizes",
        ...
        self.source_fingerprint = {
            name: {
                ...
                "mtime_ns": os.stat(path).st_mtime_ns if path and os.path.isfile(path) else None,
            }
    ...
    def _truncate_patch_ids(self, patch_ids, max_cells_per_patch, hs, ws):
        if len(patch_ids) <= max_cells_per_patch:
            return patch_ids
        logging.debug(
            "Patch (%s,%s) has %d cells; truncating to %d",
            hs,
            ws,
            len(patch_ids),
            max_cells_per_patch,
    ...
    def __getitem__(self, index):
        ...
        if self.normstain:
            try:
                ...
                if self._norm_failures < self._norm_failures_max_log:
                    logging.warning(
                        "Stain normalisation failed at patch (%s,%s): %s: %s",
                        hs,
                        ws,
                        type(exc).__name__,
                        exc,
                    ...
                    logging.warning("Further stain normalisation failures muted")
        ...
        # Preserve the historical path exactly unless the official UNI2-H
        # tensor contract is explicitly enabled. The UNI2 path stays raw here
        # so stain and joint geometric transforms occur before RGB/255 and
        ...
        if (
            ...
            hist_patch = hist_patch.astype(np.float32, copy=False)
        ...
        hist_torch = normalize_histology_tensor_after_joint_transform(
            hist_torch,
            self.foundation_model_input_normalization,
        )
