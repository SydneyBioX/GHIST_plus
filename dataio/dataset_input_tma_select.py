...
class DataProcessing(data.Dataset):
    def __init__(
        ...
        self.force_no_punch_filter = bool(force_no_punch_filter)
        self.slide_idx = int(getattr(opts_data_sources, "slide_idx", -1))
        ...
        self.coords_starts = list(coords_starts_valid)
        self.coords_starts_unfiltered = list(coords_starts_valid)
        self._apply_punch_filter_if_available()
    ...
    def _apply_punch_filter_if_available(self):
        if self.force_no_punch_filter:
            return
        enabled = bool(
            getattr(self.opts_data, "punch_select_enabled", False)
            or getattr(self.opts_data, "tma_select_enabled", False)
        )
        if not enabled:
            return
        split_cfg = str(getattr(self.opts_data, "punch_filter_splits", "train")).lower()
        allowed = {x.strip() for x in split_cfg.replace(",", " ").split() if x.strip()}
        if allowed and self.mode not in allowed:
            return
        fp_punch = _punch_cache_path(self.experiment_path, self.slide_idx)
        if not os.path.isfile(fp_punch):
            print(f"[punch] No cached punch for slide={self.slide_idx}; using full patch set.")
            return
        try:
            meta = torch.load(fp_punch, weights_only=False)
            center = meta.get("punch_center", None)
            window_px = float(meta.get("window_px", 0.0))
            if center is None or len(center) < 2 or window_px <= 0:
                print(f"[punch] Invalid punch cache {fp_punch}; using full patch set.")
                return
            cy, cx = float(center[0]), float(center[1])
            half = 0.5 * window_px
            filtered = []
            for hs, ws in self.coords_starts:
                py = float(hs) + 0.5 * float(self.hsize)
                px = float(ws) + 0.5 * float(self.wsize)
                if abs(py - cy) <= half and abs(px - cx) <= half:
                    filtered.append((hs, ws))
            if filtered:
                print(
                    f"[punch] Applied punch filter slide={self.slide_idx}: "
                    f"{len(filtered)}/{len(self.coords_starts)} patches"
                )
                self.coords_starts = filtered
            else:
                print(f"[punch] Punch filter empty for slide={self.slide_idx}; using full patch set.")
        except Exception as exc:
            print(f"[punch] Failed loading punch cache {fp_punch}: {exc}; using full patch set.")
    ...
    def __getitem__(self, index):
        ...
        if self.return_coords:
            patch_coord = torch.tensor(
                [float(hs) + 0.5 * float(self.hsize), float(ws) + 0.5 * float(self.wsize)],
                dtype=torch.float32,
            )
            patch_slide_idx = torch.tensor(self.slide_idx, dtype=torch.long)
            return sample + (patch_coord, patch_slide_idx)
        return sample
