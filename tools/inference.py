...
def _build_model(gba, opts, n_classes, n_genes, n_ref, use_avgexp, use_celltype, use_neighb, device):
    ...
    framework_cls = (
        gba.hurdle_framework.HurdleFramework
        if hurdle_enabled
        else gba.model_framework.Framework
    )
    try:
        model = framework_cls(
            n_classes,
    ...
    panel_completion_enabled = holdout_n_genes > 0 and not hurdle_enabled
...
def main():
    ...
    hurdle_enabled = bool(getattr(model, "hurdle_enabled", False))
    if hurdle_enabled:
        ...
        for batch_idx, (
            ...
            if hurdle_enabled:
                ...
                if signed_mu is None or occurrence_logits is None:
                    raise RuntimeError("Hurdle inference auxiliaries are missing")
                pred = signed_mu.detach().cpu().numpy().astype(np.float32)
                occurrence_np = occurrence_logits.detach().cpu().numpy().astype(np.float32)
            else:
            ...
            if hurdle_enabled:
                all_occurrence_logits.append(occurrence_np[valid])
    ...
    ids_raw = np.concatenate(all_ids, axis=0)
    values_raw = np.concatenate(all_preds, axis=0)
    order = np.argsort(ids_raw, kind="stable")
    ids = ids_raw[order]
    values = values_raw[order]
    uniq_ids, idx_start, counts = np.unique(ids, return_index=True, return_counts=True)
    mean_signed_mu = None
    mean_occurrence_logits = None
    mean_occurrence_probability = None
    hurdle_k_positive = None
    hurdle_effective_positive = None
    hurdle_positive_shortfall = None
    hurdle_gate_diagnostics = None
    if hurdle_enabled:
        logits_raw = np.concatenate(all_occurrence_logits, axis=0)
        unique = gba.hurdle_evaluation.aggregate_unique_hurdle_rows(
            ids_raw,
            values_raw,
            logits_raw,
        )
        (
            uniq_hurdle,
            mean_signed_mu,
            mean_occurrence_logits,
            _,
            _,
            hurdle_counts,
        ) = unique
        if not np.array_equal(uniq_ids, uniq_hurdle) or not np.array_equal(counts, hurdle_counts):
            raise RuntimeError("Hurdle and standard cell-ID aggregation disagree")
        pred_scaled = gba.hurdle_evaluation.cohort_gate_numpy(
            mean_signed_mu, mean_occurrence_logits
        ).astype(np.float32)
        mean_occurrence_probability = (
            1.0 / (1.0 + np.exp(-np.clip(mean_occurrence_logits, -80.0, 80.0)))
        ).astype(np.float32)
        (
            hurdle_k_positive,
            hurdle_effective_positive,
        ) = gba.hurdle_evaluation.hurdle_gate_counts(
            pred_scaled, mean_occurrence_logits
        )
        hurdle_positive_shortfall = hurdle_k_positive - hurdle_effective_positive
        hurdle_gate_diagnostics = (
            gba.hurdle_evaluation.summarize_hurdle_gate_counts(
                hurdle_k_positive, hurdle_effective_positive
            )
        )
        aggregation_mode = "mean_q_and_signed_mu_then_unique_slide_topk"
    else:
        sum_preds = np.add.reduceat(values, idx_start, axis=0)
    ...
    if hurdle_enabled:
        _log(
            "[INFO] hurdle positives effective/requested=%d/%d shortfall=%d genes_with_shortfall=%d"
            % (
                hurdle_gate_diagnostics["hurdle_effective_positive_total"],
                hurdle_gate_diagnostics["hurdle_requested_positive_total"],
                hurdle_gate_diagnostics["hurdle_positive_shortfall_total"],
                hurdle_gate_diagnostics["hurdle_genes_with_positive_shortfall"],
            )
        )
    ...
    if hurdle_enabled:
        npz_payload["signed_mu_scaled"] = mean_signed_mu.astype(np.float32)
        npz_payload["occurrence_logits"] = mean_occurrence_logits.astype(np.float32)
        npz_payload["occurrence_probability"] = mean_occurrence_probability.astype(np.float32)
        npz_payload["hurdle_k_positive"] = hurdle_k_positive.astype(np.int64)
        npz_payload["hurdle_effective_positive"] = hurdle_effective_positive.astype(np.int64)
        npz_payload["hurdle_positive_shortfall"] = hurdle_positive_shortfall.astype(np.int64)
        npz_payload["hurdle_gate_rule"] = np.asarray(
            "K_g=round(sum_i q_ig); top-K q; gate*ReLU(signed_mu)", dtype=object
        )
    ...
    if has_eval_targets and not args.skip_metrics:
        ...
        evaluation_function = (
            gba.hurdle_evaluation.evaluate_hurdle_validation
            if hurdle_enabled
            else gba.evaluation_utils.evaluate_validation
        )
        evaluation_extra = ({"expr_scale": float(expr_scale)} if hurdle_enabled else {})
        _log("[INFO] computing metrics on the exported unique-cell prediction rule")
    ...
    meta = {
        ...
        "hurdle_gate_rule": (
            "K_g=round(sum_i q_ig); top-K q; gate*ReLU(signed_mu)"
            if hurdle_enabled else None
        ),
        "hurdle_gate_diagnostics": hurdle_gate_diagnostics,
        "n_genes": int(n_genes),
