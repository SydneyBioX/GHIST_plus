...
- **Plot Figure 2:** follow the [R workflow](#figure-2-roi-and-pathway-analysis-r).
...
## Figure 2: ROI and pathway analysis (R)

This workflow plots the Figure 2 ROI pathway maps, top-pathway bars, validation
summaries, and pathway correlations from prepared data tables. Keep the three
R files together in the repository root:

| File | Role | When to use it |
| --- | --- | --- |
| [Figure2.R](Figure2.R) | Reads prepared CSVs and exports the individual plots and combined layout. | Main plotting entry point. |
| [plot_roi_pathways.R](plot_roi_pathways.R) | Uses precomputed single-cell pathway scores and spatial data to prepare the two ROI pathway tables and plots. | Optional preparation step when those tables need to be regenerated. |
| [figure_plot_utils.R](figure_plot_utils.R) | Defines shared styles, colours, spatial scale bars, and export functions. | Loaded automatically by `Figure2.R`; do not run it separately. |

The Figure 2 CSV and RDS inputs are not included in this Git repository. Supply
the matching study files separately before running the scripts. These R scripts
do not download data or use `GHIST_BUNDLE_ROOT`.

### Plot from prepared tables

Use an R installation with Cairo PDF support and the packages `tidyverse`,
`patchwork`, `tidytext`, `ggrastr`, and `shadowtext`. The Python environment in
[Installation](#installation) does not install these R dependencies.

Place the four input tables at these paths relative to the repository root:

```text
in/Figure2/
  figure4a2_IL2_STAT5_SIGNALING.csv
  figure4a3_top_pathway.csv
  2b.csv
  2c.csv
```

The first two files contain the ROI spatial pathway scores and top-pathway
summaries. `2b.csv` contains validation correlation summaries; `2c.csv` contains
the pathway correlation comparisons. Once the inputs and dependencies are
...
Outputs are written to `out/revision/`, including individual panel PDFs,
plot-data CSVs, and `figure4_combined.pdf`. The `figure4*` input and output names
retain an earlier figure numbering scheme and refer to this Figure 2 workflow.
The current [Figure4.ipynb](Figure4.ipynb) instead covers gene imputation and
VQ/composition ablation.

### Regenerate the pathway tables (optional)

Skip this step when the two prepared pathway CSVs are already available.
`plot_roi_pathways.R` requires these existing RDS objects:

- `sceGT_enrichment_ssgsea.rds`: precomputed single-cell ssGSEA pathway scores.
- `breastcancer_slides2_sce.rds`: the spatial single-cell object with matching
  cell IDs and coordinates.

In addition to the plotting packages above, the preparation script loads
`SingleCellExperiment`, `SpatialExperiment`, `scater`, `S4Vectors`, `Matrix`,
and `viridis`. It matches cells between the two objects, selects three predefined
ROIs, and ranks the top 10 pathways per ROI by median ssGSEA score. It uses
existing enrichment scores rather than computing ssGSEA. The ROI coordinates
and the pixel calibration in `figure_plot_utils.R` are specific to the study
data and must be reviewed before using another dataset.

Before running this script, set its `readRDS`, `write.csv`, and `ggsave` paths
for your data location. Its current `../out/` paths are relative to the R
working directory. After configuring those paths, run:

```bash
Rscript plot_roi_pathways.R
```

The script writes `figure4a2_IL2_STAT5_SIGNALING.csv` and
`figure4a3_top_pathway.csv`, together with the two pathway PDFs. Place the CSVs
in `in/Figure2/` before running `Figure2.R`; they are not moved there
automatically. Obtain `2b.csv` and `2c.csv` separately, because this preparation
script does not generate them.

The historical column name `mean_score` contains **median** scores in tables
generated by `plot_roi_pathways.R`. `Figure2.R` currently labels this axis
"Mean ssGSEA score"; change that label to "Median ssGSEA score" when using
these generated tables. The preparation script's own plot already uses the
median label.
